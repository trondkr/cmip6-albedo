...
class Config_albedo():
    ...
    def setup_parameters(self):
        ...
        wavelengths = wl["λ"].values
        refractive_indexes = wl["n(λ)"].values
        alpha_chl = wl["a_chl(λ)"].values
        alpha_w = wl["a_w(λ)"].values
        beta_w = wl["b_w(λ)"].values
        alpha_wc = wl["a_wc(λ)"].values
        solar_energy = wl["E(λ)"].values
        print(wl.head())
